...
void simplify_path()
{
    ...
    if(path_length < 3 || path[path_length-2] != 'B')
        return;
    if(path_length > 5 && path[path_length-2]==path[path_length-3]==path[path_length-4]==path[path_length-5])
    {
        int total_angle = 0;
        int i;
        for(i=1;i<=6;i++)
        ...
        {
            case 0:
                path[path_length - 6] = 'S';
                break;
            case 90:
                path[path_length - 6] = 'R';
                break;
            case 180:
                path[path_length - 6] = 'B';
                break;
            case 270:
                path[path_length - 6] = 'L';
                break;
        }
        // The path is now two steps shorter.
        path_length -= 5;
