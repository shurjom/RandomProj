...
if __name__ == "__main__":
    block = Block(None, 0, 0, x2=5, y2=5)
    assert block.x == 0
    assert block.y == 0
    assert block.type == 'Block'
    assert block.x2 == 5
    assert block.y2 == 5
    assert block.to_dictionary() == {'x': 0, 'x2': 5, 'parallax': 1, 'surface': None, 'block_type': 'Block', 'y': 0, 'y2': 5, 'size': 1.0}
    block.make(2,2)
    assert (block.x2, block.y2) == (2,2)
    assert (block.width, block.height) == (2,2)
